...
public class UserFindControllerImpl implements UserFindController{
    ...
    @Override
    @GetMapping("/list/addlist/{drink_id}/{drink_name}")
    ...
    @Override
    @GetMapping("/list/dellist/{drink_id}")
    public ResponseEntity<ApiResponseCreator<String>> Del_User_Find_List(@LoginUser Long user_id, Long drink_id) {
        return userFindService.Del_User_List(user_id,drink_id);
    }

    @Override
    @GetMapping("/list/log_list")
