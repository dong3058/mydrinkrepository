...
public interface UserAdminController {
    ...
    ResponseEntity<ApiResponseCreator<MemberInfoDto>> Show_Member_Data(MemberDto memberDto);

    @Operation(summary = "유저 활동로그 즉 해당 유저가 달아놓은 댓글 목록 혹은 글 보여주기.",description = "get메서드로 진행")
    ResponseEntity<ApiResponseCreator<List<DrinkCommentShowDto>>> Show_Member_Log(Long member_id);

    @Operation(summary = "유저 밴 하는 api",description = "post 메서드로 진행")
    ResponseEntity<ApiResponseCreator<String>> Change_User(Long member_id,MemberInfoDto memberInfoDto);
